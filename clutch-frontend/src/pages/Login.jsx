 ...
 * 2. Uses the JWT to fetch the user's full profile (Role, Name, ID) before routing.
 */
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Extract just the token (Java backend doesn't send ID here)
      ...
      // We immediately use the new token to fetch the user's authoritative role, name, and ID
      const profileRes = await axios.get('/api/v1/users/me', {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      // Store display and routing context (Grabbing the ID from here now!)
      localStorage.setItem('clutch_userId', profileRes.data.id); 
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="bg-surface w-full max-w-md p-8 rounded-2xl shadow-soft">
        ...
        <form onSubmit={handleLogin}>
          <InputField label="University Email" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="rollnumber.cse@andhrauniversity.edu.in" />
